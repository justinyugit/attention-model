...
class StateCVRP(NamedTuple):
    # Fixed input
    coords: torch.Tensor  # Depot + loc #### Every Grid ID or subarea
    ...
    def initialize(input, visited_dtype=torch.uint8): ## only run once per instance of the cvrp?
        ...
        return StateCVRP(
            ...
            demand=demand, ## dont need
            ids=torch.arange(batch_size, dtype=torch.int64, device=loc.device)[:, None],  # Add steps dimension
            prev_a=torch.zeros(batch_size, 1, dtype=torch.long, device=loc.device),
            used_capacity=demand.new_zeros(batch_size, 1), ## related to demand can be removed?
            ...
            cur_coord=input['depot'][:, None, :],  # Add step dimension ## set cur_coord to the station due to starting point
    ...
    def update(self, selected):
        ...
        # )[:, 0, :]
        lengths = self.lengths + (cur_coord - self.cur_coord).norm(p=2, dim=-1)  # (batch_dim, 1) ## finds the distance between the prev and curr coord
    ...
    #################################################### which locations to not visit, need to change
    def get_mask(self):
        """
        ...
        exceeds_cap = (self.demand[self.ids, :] + self.used_capacity[:, :, None] > self.VEHICLE_CAPACITY) ## need to change this to remove demand
        # Nodes that cannot be visited are already visited or too much demand to be served now
        mask_loc = visited_loc.to(exceeds_cap.dtype) | exceeds_cap ## apply bitwise OR 
